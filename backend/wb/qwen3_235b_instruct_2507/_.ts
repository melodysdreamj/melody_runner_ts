...
export async function requestCloudflareGatewayWbQwen3(
    ...
    // Try to load from default location first, then fall back to relative path
    dotenv.config();
    ...
    // Target Path: v1
    // Cloudflare Gateway requires 'Custom Provider' setup for W&B, which appears to be missing (Error 2008).
    // Switching to Direct W&B Inference API for immediate functionality.
    // const baseURL = `https://gateway.ai.cloudflare.com/v1/${accountId}/${gatewayName}/${WANDB_TARGET_HOST}/v1`;
    const baseURL = `https://${WANDB_TARGET_HOST}/v1`;
