// 쿠팡 + 알리익스프레스 대분류 카테고리 CSV 데이터 생성 스크립트
// 실행: npx ts-node backend/csv/hot_category/data/generate.ts
...
// CSV 파일 생성
const headers = ["vendor", "categoryId", "categoryName", "itemCount", "categoryLevel"];
const rows = categories.map((c) =>
  [c.vendor, c.categoryId, c.categoryName, c.itemCount, c.categoryLevel]
    .map(escapeCsvValue)
    .join(",")
);
const csvContent = [headers.join(","), ...rows].join("\n") + "\n";

const outputPath = path.join(__dirname, "hot_categories.csv");
fs.writeFileSync(outputPath, csvContent, "utf-8");

console.log(`✅ CSV 파일 생성 완료: ${outputPath}`);
console.log(`   쿠팡: ${categories.filter(c => c.vendor === "coupang").length}개 대분류`);
